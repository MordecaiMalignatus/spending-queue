...
fn delete_head_item() -> Result<()> {
    let mut state = read_file();
    match state.future_purchases.pop_front() {
        Some(item) => {
...
fn display_queue() -> Result<()> {
    ...
    state.future_purchases.iter().for_each(|item| {
        ...
        let name = match item.purchase_link.is_some() {
            true => Style::new().italic().paint(item.name.clone()),
            false => item.name.clone().into(),
        };
        println!("{}\t{}", name, cost);
    });
...
fn add_to_purchase_queue(thing_to_add: String, prepend: bool) -> Result<()> {
    ...
    match stdin.read_line(&mut purchase_url) {
        Ok(_bytes_read) => {}
        Err(_) => {
            panic!("Can't parse purchase url, aborting...");
        }
    }
...
fn update_budget(amount: f64, interval: u8) -> Result<()> {
    let mut state = read_file();
